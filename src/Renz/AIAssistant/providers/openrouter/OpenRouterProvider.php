...
class OpenRouterProvider implements AIProvider {
    ...
    public function generateResponse(string $query, array $conversationHistory, string $systemPrompt, string $serverInfo, string $serverFeatures = ""): string {
        ...
        try {
            ...
            $response = $this->makeRequest("https://openrouter.ai/api/v1/chat/completions", $data);
            ...
            if (isset($responseData["choices"][0]["message"]["content"])) {
                ...
                if ($this->plugin->isDebugEnabled() && isset($responseData["error"])) {
                    $this->plugin->getLogger()->debug("OpenRouter API error: " . json_encode($responseData["error"]));
                }
    ...
    private function makeRequest(string $url, array $data): string {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            ...
            "HTTP-Referer: https://pmmp-ai.plugin",
            "X-Title: PMMP-AI Plugin",
    ...
    public function isConfigured(): bool {
        return !empty($this->apiKey) && $this->apiKey !== "YOUR_OPENROUTER_API_KEY";
    }
}